...
namespace SundanceStdMesh
{
  namespace Internal
  {
    ...
    /**
     * VertexView is a read-only "view" of a cell's vertices, where the vertices are
     * stored contiguously in a large master array. By working with views, we can
     * greatly reduce the number of temporary arrays created during hashtable searches
     * for existing vertex arrays.
     */
    class VertexView
    {
    public:
       ...
       * \param bese pointer to the start of the master data array. By using double
       * indirection, the master array can be resized or relocated and VertexViews
       * can remain valid. 
     ...
     * Return a hash code for the vertex set. 
     */
    inline int VertexView::hashCode() const
    {
      int rtn = 0;
      int* p = *base_ + offset_*length_;
