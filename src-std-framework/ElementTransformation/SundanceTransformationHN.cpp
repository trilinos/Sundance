...
void TransformationHN::preApply(
	...
	for (i = 0 ; i < cellLIDs->size() ; i++){
		 ...
		 dofMap_->getTrafoMatrixForCell( cellLID , matrixSize, doTransform, M );
	     if (doTransform){
...
/** */
void TransformationHN::postApply(
		const int multiplicity ,
		const int vectorPerCell,
		const CellJacobianBatch& JTrans,
	    const CellJacobianBatch& JVol,
...
void TransformationHN::preapplyTranspose(
		const int multiplicity ,
		const int vectorPerCell,
		const int cellDim,
